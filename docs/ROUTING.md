...
| `shop-hub`, 8 category, 6 concern, 23 `shop-product-*` (all client-approved SkinMedica products, full bilingual detail content), cart, checkout, shipping-returns (41) | `shopEnabled` | Product data and content are approved, imported, and validated (`src/content/products.ts`, `tests/unit/skinmedica-catalogue.spec.ts`); the remaining blocker is product photography — see `docs/MEDIA.md` |
