...
| ~~GAP-008~~ | ~~23 `shop-product-*`~~ | ~~Approved, uploaded SkinMedica product photography~~ | **WITHDRAWN 2026-09-07 — the gap no longer exists because the line no longer publishes.** Blue Diamond stopped carrying SkinMedica and the catalogue is Myriade only, so the 4 records that still had no approved packshot (`lytera-2-pigment-brightening-serum`, `daily-physical-defense-spf-34`, `total-defence-repair-spf-34-tinted`, `replenish-hydrating-cream`) are archived along with the other 19. Nothing is owed by the client here. The 23 records are preserved verbatim in `src/features/products/archive/skinmedica.ts` and readable at `docs/archive/SKINMEDICA_ARCHIVE.md`; if the line ever returns, this gap returns with it and reopens at 4 of 23, not at 23 of 23 — the 19 approved assignments still exist in FeelStack | n/a | No SkinMedica page publishes | — withdrawn |
