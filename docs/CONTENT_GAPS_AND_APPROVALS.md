...
| GAP-008 | 23 `shop-product-*` | Approved, uploaded SkinMedica product photography | Every product record is complete (name, price, size, descriptions, category, usage, sources), **Resolved for 19 of 23.** The FeelStack media library holds 21 SkinMedica packshots under `/blue-diamond/shop/`; 19 are approved and assigned to their product as `productPrimary`, and those 19 render their real photograph on the catalogue and detail pages in both locales. The 4 below have no approved packshot and render the neutral placeholder. A sibling product's photograph is never substituted — the CLEAR sunscreen standing in for the TINTED one would be a product misrepresentation. Remaining: `lytera-2-pigment-brightening-serum`, `daily-physical-defense-spf-34`, `total-defence-repair-spf-34-tinted`, `replenish-hydrating-cream`. Supplying one is a CMS action — upload, approve, assign, publish — with no code change | Yes | Approved neutral placeholder renders for all 23 | Client + internal (ImageKit credentials) |
...
| GAP-010 | `booking:walk-in` | Whether Skip the Waiting Room is still in use | `ab.skipthewaitingroom.com` appears in the source but is not in the approved booking allowlist | Yes | CL-007: walk-in booking now routes through the clinic-specific Skip the Waiting Room channel, whose URL has not been supplied and is `null` in `src/config/booking.ts`. **No URL invented**; every booking href resolves through the centralized typed booking config | Client |
| GAP-017 | `medical-after-hours-care` FAQ 1 | The exact wording of a NEW fifth sentence for "What counts as an urgent, non-emergency need?" | CL-014 asks for a sentence immediately after the one ending "emergency department, not this pathway." and does not supply it. **Nothing was invented or paraphrased** | Yes | The approved answer publishes unchanged; the insertion point is annotated in `src/features/medical-services/data.ts`. Append the approved sentence to the `en` string there | Client — `CLIENT_CLARIFICATION_REQUIRED` |
| GAP-018 | `shop-product-purifying-peeling` | Approved product packaging photograph | CL-037 supplied the full text record (title, subtitle, price, Benefits, Key features) but no image. A treatment/equipment photograph is **not** a substitute | Yes, as catalogue content only | Published non-purchasable: `purchaseBlocked` renders an explicit note, no purchase action, no availability claim | Client |
| GAP-019 | `shop-product-brightening-peeling` | Approved product packaging photograph **and** price | CL-038 supplied the text record only. The Purifying peel's `188 + GST` is **not** reused and no price is estimated | Yes, as catalogue content only | Published non-purchasable with `priceCents: null` — no price line renders at all | Client |
| GAP-020 | 8 Aesthetics treatment/technology routes | The eight original treatment images named in CL-039 – CL-041 | Every supplied path is rooted at a user profile absent from the build machine; the originals are not in ImageKit or FeelStack. Full filename → route → slot table in `docs/MEDIA.md` | Yes | Affected slots render the seeded `FacetTile` brand fallback — a designed stand-in, never a wrong device photograph | Client |
| GAP-021 | Global brand mark | Nothing missing — **resolved 2026-09-07** | The client supplied the logo directly as an 800×661 PNG lock-up (not the register's filename, which was never on the build machine), superseding the 1536×1024 black-field JPEG sent 2026-09-06. It is stored verbatim at `src/assets/brand/blue-diamond-medical-logo.png` | Yes | `Logo`/`BrandLockup` render the mark cut from it (`src/assets/brand/blue-diamond-medical-mark.png`, the diamond on transparency); the inline `DiamondMark` SVG stand-in is deleted, and the favicon is rebuilt from it. See `docs/MEDIA.md` CL-001 | — resolved |
...
| GAP-022 | `careers` | A private application backend: a delivery provider, a non-public document store, and a captcha credential pair | The legacy `/join-our-team` form collected Name/Phone/Email/**Attach Resume** behind reCAPTCHA. A résumé is employment data — it cannot go to ImageKit or the FeelStack media library (both public asset stores), and this build has no configured `CONTACT_DELIVERY_PROVIDER` (`src/lib/forms/delivery.ts`), no private storage, and no captcha keys. Standing one up ad hoc would be worse than having no form | Yes | Page publishes in **email-only** mode: the approved recruitment copy plus a `mailto:` to `accountant@bluediamondmedical.ca`, which is where the legacy form delivered anyway. No upload control renders, and `tests/e2e/careers.spec.ts` fails if one is added without a backend. **No vacancy, salary, employment type, benefit or deadline is published** — none was supplied | Client + whoever provisions the delivery/storage/captcha credentials |
| GAP-023 | `careers` (Arabic) | Approved Arabic for the new English recruitment sections | The client approved new English copy only (eyebrow, H1, intro, "Join Our Team" body, application and privacy sections). Machine-translating recruitment copy is what `docs/CONTENT_MODEL.md` forbids | Yes | `/ar/الوظائف` keeps the Arabic it already published and omits the un-translated sections, exactly as `/about` handles its aesthetics block. No English text renders under `/ar` | Client |
...
|---|---|---|---|---|---|---|
| CFG-001 | `doctor-saeed` | Missing booking CTA on the legacy page — the only physician without one | Could be a deliberate omission (not accepting bookings) or a legacy oversight. **No booking URL was invented** | Yes | Currently assigned the shared `family-doctor` (Mikata) channel used by every other physician. Confirm this is correct | Client |
