...
| GAP-021 | Global brand mark | Nothing missing — **resolved 2026-09-06** | The client supplied the mark directly as a 1536×1024 JPEG (not the register's filename, which was never on the build machine). It is stored verbatim at `src/assets/brand/blue-diamond-logo-source.jpg` | Yes | `Logo`/`BrandLockup` render the supplied mark (`src/assets/brand/blue-diamond-mark.png`, the diamond cut out onto transparency); the inline `DiamondMark` SVG stand-in is deleted, and the scaffold favicon was rebuilt from it. See `docs/MEDIA.md` CL-001 | — resolved |
