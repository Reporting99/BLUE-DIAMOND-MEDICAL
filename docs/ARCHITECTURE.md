...
|---|---|---|---|
| `family-doctor` | Mikata | `https://app.mikatahealth.com/book-appointment?token=it7K7LTw7OHcbe` | Book with your family doctor (header, homepage, `/medical`) |
| `walk-in` | Skip the Waiting Room | `https://app.mikatahealth.com/book-appointment?token=it7K7LTw7OHcbe` | "Skip the Waiting Room" walk-in / new-patient booking |
...
- `allowedBookingHosts` (`app.mikatahealth.com`, `mikatahealth.com`, `euclidtelehealth.org`, `skipthewaitingroom.com`) is validated by `src/lib/security/booking-allowlist.ts` — any future channel pointing outside this allowlist fails a check rather than silently linking out to an unreviewed host.
...
The brief allows Arabic pages to note that an external booking system may open in English. This was checked, not assumed: the legacy `mika.care` link redirects (server-side, on Mikata's own infrastructure) to `chat.mikatahealth.com`, which could not be confirmed either way for Arabic support via an automated fetch (likely a JS-rendered app). No approved source confirms or denies Arabic support for any of the three providers (Mikata, Euclid Telehealth, Skip the Waiting Room).
...
- No booking data is stored, logged, or transmitted anywhere by this codebase — the three providers (Mikata, Euclid, Skip the Waiting Room) each run their own HIPAA/PIPEDA-appropriate systems independently of this site.
