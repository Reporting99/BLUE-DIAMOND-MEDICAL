...
| `/blue-diamond/brand/blue-diamond-medical-mark.png` | Every route | Header lock-up, footer lock-up, About hero lock-up | Blue Diamond Medical Clinic | عيادة بلو دايموند الطبية | 424×519 | — | **approved** — cut from the lock-up the client supplied 2026-09-07, imported via FeelStack `media/import` the same day, sha256 `34557546…f10d4`, CDN original verified byte-identical with `?tr=orig-true`. Rendered through the `logo` preset by `src/components/layout/Logo.tsx`, not `ImageKitImage` |
| `/blue-diamond/brand/blue-diamond-medical-logo.png` | — | Provenance only; nothing renders it | Blue Diamond Medical logo | شعار بلو دايموند الطبية | 800×661 | — | The client's supplied lock-up, byte-for-byte (sha256 `7ea7355d…5c0c`). Kept so the mark above can be re-derived without asking for the file again |
...
<!-- `/blue-diamond/shop/skinmedica-collection.jpg` (Shop hub, Overview) was here. Removed 2026-09-07 with the SkinMedica line: it was a `pending` slot for a photograph of a collection the clinic no longer carries, so it was a standing request to shoot an asset nothing could render. See docs/archive/SKINMEDICA_ARCHIVE.md. -->
...
- **SkinMedica product photography (23 SKUs)** — **no longer required (2026-09-07).** The line is archived (`skinMedicaEnabled: false`) and no SkinMedica page publishes, so the 4 outstanding packshots are not owed by anyone; GAP-008 is withdrawn. The 19 approved `productPrimary` assignments still exist in FeelStack and are untouched, so if the line ever returns the imagery returns with it. See `docs/archive/SKINMEDICA_ARCHIVE.md`.
...
- **Logo** — cut from the client's own supplied file, **served from ImageKit** at `/blue-diamond/brand/blue-diamond-medical-mark.png` (imported 2026-09-07 on the client's instruction) and no longer the inline SVG recreation. The one asset that does NOT go through `ImageKitImage`: the mark resolves in `src/lib/media/brand-mark.ts`, which falls back to the copy bundled at `src/assets/brand/blue-diamond-medical-mark.png` if the manifest entry is ever set back to `pending` — a FacetTile in place of the logo would read as a broken header on every route, which is not a graceful degradation. See CL-001 below.
...
| File | Reason |
|---|---|
| `medical/blue-diamond-medical-logo.png` | Low-resolution legacy web logo; superseded by the lock-up the client supplied on 2026-09-07, now bundled at `src/assets/brand/blue-diamond-medical-logo.png`. Its ImageKit copy at `/blue-diamond/shared/brand/68406a8a540e-…` is queued for deletion — the import identity is not permitted to remove media |
...
| Logo (header/footer) | — | Diamond + heartbeat mark | `/blue-diamond/brand/blue-diamond-medical-mark.png` | — | **supplied, imported and in place** | The client supplied the full lock-up on 2026-09-07 and asked for it to be served from ImageKit; `src/components/layout/Logo.tsx` renders the CDN copy through the `logo` preset, with the committed original as the build-time fallback. Still a raster — Decca Design Inc.'s master vector file (SVG/EPS) supersedes it if it ever arrives. |
...
| ~~SkinMedica products (23, `/shop/*`)~~ | Product photography | Bottle/packaging shots | `/blue-diamond/shop/<catalogue-number>_<Product_Name>.jpg`, assigned in FeelStack | 1:1 | **archived 2026-09-07 — not published** | The line was withdrawn; the assets stay in the FeelStack library. The live catalogue is Myriade, whose 31 packshots are real approved assets carried on the product records themselves under `/blue-diamond/products/myriade/` |
...
| --- | --- | --- | --- | --- |
| CL-001 | ~~`Blue Diamond Medical Clinic-logo2024-01-15_22-09-36-b10847c6-org-427.png`~~ — **RE-SUPPLIED 2026-09-07** as an 800×661 PNG lock-up attached directly (`sha256 7ea7355d…`, kept verbatim at `src/assets/brand/blue-diamond-medical-logo.png`). It replaces the 1536×1024 black-field JPEG supplied 2026-09-06 (`sha256 5a97518c…`), which is deleted along with the mark cut from it | global brand mark | `src/components/layout/Logo.tsx` | **Done.** `DiamondMark` renders `src/assets/brand/blue-diamond-medical-mark.png` — the diamond cut out of the supplied lock-up onto transparency, aspect ratio preserved (424×519), interior white kept — in `Logo` **and** `BrandLockup` (header, mobile nav, footer, hero lock-up). The live bilingual wordmark is kept in preference to the file's English-only one, so Arabic routes still read بلو دايموند الطبية. Derivation and checksums: `src/assets/brand/README.md`. **Served from ImageKit** at `/blue-diamond/brand/blue-diamond-medical-mark.png` (media asset `985bde55-…b671ea`, CMS row `pending` as every import is; the manifest entry is the approval that matters here). |
