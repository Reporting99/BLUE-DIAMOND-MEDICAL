import type { Product, ProductBrand, ProductSource } from "@/features/products/types";

/**
 * ARCHIVE — the 23 client-approved SkinMedica records, retired 2026-09-07.
 *
 * Blue Diamond stopped carrying SkinMedica and now carries Myriade only, so
 * this catalogue no longer publishes: nothing here reaches `products`,
 * `src/config/routes.ts`, the sitemap or any rendered page while
 * `features.skinMedicaEnabled` is `false` (src/config/features.ts).
 *
 * It is ARCHIVED, not deleted, because the client's instruction was that the
 * line is stopped "currently". The records keep their full approved shape —
 * exact names, prices, sizes, bilingual detail copy, per-product FAQs and the
 * `sources` trace behind every claim — so restoring the line is one flag flip
 * (`skinMedicaEnabled: true`) and a deploy, with no content rewritten and no
 * research repeated. Data.ts re-inserts these ahead of the Myriade records in
 * that case, which is the order the catalogue published in until now.
 *
 * The three helpers below moved here with the records they serve: every one
 * of them is SkinMedica-specific (a SkinMedica packshot placeholder, the
 * SkinMedica manufacturer source, and the authorized-Canadian-SkinMedica-
 * retailer source), so leaving them in data.ts would have left the live
 * catalogue importing vocabulary for a brand it no longer carries.
 *
 * A human-readable export of everything in this file — products, prices,
 * sizes, FAQs and sources — is docs/archive/SKINMEDICA_ARCHIVE.md.
 *
 * Do not add to this file. It is a frozen record of what was approved, not a
 * catalogue under maintenance.
 */

/** The brand record itself, out of `productBrands` for the same reason. */
export const skinMedicaBrand: ProductBrand = { id: "skinmedica", slug: "skinmedica", slugAr: "سكين-ميديكا", name: "SkinMedica" };
...
/** The 23 approved SkinMedica records, verbatim as last published. */
export const archivedSkinMedicaProducts: Product[] = [
  // ============ THE GROWTH FACTOR ============
  {
    id: "lumivive-system",
